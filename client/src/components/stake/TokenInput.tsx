...
const tokenIcons: Record<string, React.ReactNode> = {
  gUSD: <div className="w-5 h-5 rounded-full bg-gradient-to-br from-orange-500 to-orange-400" />,
  gETH: <div className="w-5 h-5 rounded-full bg-gradient-to-r from-orange-400 to-orange-500" />,
};
            ...
            <div className="absolute top-full right-0 mt-1.5 w-44 bg-card border border-border rounded-xl shadow-xl z-50 overflow-hidden py-1">
              {COINS.map((coin) => (
                ...
                >
                  ...
                  {token === coin.symbol && <Check className="h-3.5 w-3.5 text-[#ff6a1a]" />}
                </button>
              ))}
            </div>
          )}
